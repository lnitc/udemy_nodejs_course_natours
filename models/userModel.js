...
userSchema.pre('save', function (next) {
  if (!this.isModified('password') || this.isNew) return next(); //only run this function if password was actually modified
  this.passwordChangedAt = Date.now() - 1000; //subtract 1 second to make sure it is in the past
  next();
});

userSchema.pre(/^find/, function (next) {
  //this points to the current query
  this.find({ active: { $ne: false } }); //exclude inactive users from results
  next();
});
...
userSchema.methods.createPasswordResetToken = function () {
  //reset token is a kind of temporary password the user will use to reset their password
  //NB: never store the reset token in the database in plain text
  const resetToken = crypto.randomBytes(32).toString('hex');

  //we hash the reset token to make it unusable in case someone breaks into the database
